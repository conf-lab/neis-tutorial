...
// Framework-agnostic core of the /api/chat endpoint, shared by the local
// Express dev server (server.ts) and the Vercel serverless function (api/chat.ts).
export async function handleChatRequest(body: any): Promise<ChatHandlerResult> {
  const { message, history = [], currentMenu = "", apiKey } = body || {};

  if (!message || typeof message !== "string") {
    return { status: 400, json: { error: "메시지를 입력해주세요." } };
  }
  ...
  try {
    ...
    const invalidKey =
      status === 401 ||
      status === 403 ||
      /API_KEY_INVALID|API key not valid|PERMISSION_DENIED/i.test(errMessage);
